...
dataset_name = 'product_and_user_final.npz'
...
def Evaluation(step):
    print '...'
    U=sess.run(thetau)
    I=np.zeros([itemnum,K],dtype=np.float32) # Each image in latent space
...
while step * batch_size <= training_epoch*oneiteration+1:
    ...
    if step*batch_size / oneiteration >epoch:
        ...
        f.flush()
    print("Percent complete: " + str(100 * (step * batch_size) / float((training_epoch*oneiteration+1))))
    step += 1
