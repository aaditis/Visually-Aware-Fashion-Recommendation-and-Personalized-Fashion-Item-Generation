...
class DCGAN(object):
  def __init__(self, sess, input_height=64, input_width=64, crop=True,
    ...
    with tf.variable_scope('GAN'):
        ...
        _,_,_,self.data,_,_=np.load('../'+self.dataset_name, allow_pickle=True, encoding='bytes')
  ...
  def train(self, config):
    ...
    self.g_sum = image_merge_summary([self.z_sum, self.d__sum,
      self.G_sum, self.d_loss_fake_sum, self.g_loss_sum])
    self.d_sum = image_merge_summary(
